<template>
  <div class="chat-box">
    ...
    <div class="user-item" @click="changeuser(i,index)" v-for="(i,index) in userlist" :key="index">
  ...
  methods: {
    ...
    getHistory(){
      request({
        ...
        for(let i=0;i<len;i++){
          let item=JSON.parse(data.data[i].msg);
    ...
    getUserList(){
      request({
        ...
        for(let i=0;i<keys.length;i++){
          let item=mp[keys[i]];
          let it=item[item.length-1];
          let msg=JSON.parse(it.msg);
    ...
    initWebSocket() {
      ...
      if (window.WebSocket) {
        let ws = new WebSocket(process.env.VUE_APP_BASE_API.replace("http","ws")+"/pushMessage/"+-this.userId+"/apctoken");
        _this.ws = ws;
        ...
        ws.onmessage = function(e) {
          ...
          if (resData.funName == "userCount") {
            ...
            _this.list = [
              ..._this.list,
              { userId: -resData.userId, msg: resData.content }
            ];
            setTimeout(() => {
              _this.scrollBottm()
            }, 5);
